...
class Mapa:
    ...
    def mover_agente_inteligente(self, red_neuronal):
        """
        El agente se mueve únicamente con base en la decisión local de la red neuronal.
        La red produce [giro, direccion].
        ...
        self.orientacion = (self.orientacion + giro) % 4

        dr, dc = VECTORES_DIRECCION[self.orientacion]
        if direccion == -1:
            dr, dc = -dr, -dc

        nueva_posicion = [
            self.posicion_agente[0] + dr,
            self.posicion_agente[1] + dc,
        ]
...
def main():
    DIMENSION_MINIMA = 5
    DIMENSION_MAXIMA = 10
    DATASET = Path("00.data/agente_cuatro_puntos_filtrado.csv")
